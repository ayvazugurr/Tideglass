(() => {
  ...
  function menu() {
    ...
    pointer = null;
    show(
      `<div class="cover-art"><img src="assets/tideglass-cover-v1.webp" alt="" width="720" height="1080"><div class="cover-copy"><p class="eyebrow">KÜÇÜK BİR OKYANUS. BÜYÜK BİR ZİNCİR.</p><h1 id="panel-title"><span>TIDE</span>GLASS<small>BİR DALGA DAHA</small></h1></div></div><p class="cover-lead">Akışını bul. <strong>Dalgayı durdur.</strong><br>Aynı renk kristalleri birleştir; uzun zincirlerle kendine alan aç.</p><div class="tip-grid"><div><b>3+</b>birleştir ve topla</div><div><b>6+</b>dalgayı durdur</div><div><b>18</b>dalgakıranı doldur</div></div>${activeRun ? '<button class="primary" id="continue-run">KALDIĞIN YERDEN DEVAM ↗</button>' : ""}<button class="${activeRun ? "secondary" : "primary"}" id="start">${started ? "YENİ SEFERE ÇIK" : "AKIŞA KATIL"} &nbsp; ↗</button><div class="palette-row">${themes.map((t, i) => `<button class="palette ${saved.theme === i ? "selected" : ""}" data-theme="${i}" ${saved.best < t.at ? "disabled" : ""}><span class="swatches" aria-hidden="true">${t.colors.map((c) => `<i style="background:${c}"></i>`).join("")}</span>${t.name}<br>${saved.best < t.at ? t.at + " puan" : "◈"}</button>`).join("")}</div><button class="secondary" id="journal">KAPTAN GÜNLÜĞÜ · İLERLEME</button><p class="fine">${rank()} · ${saved.runs} SEFER · ${saved.loops} HALKA<br>${saved.best ? "REKOR " + saved.best.toLocaleString("tr-TR") + " · " : ""}Reklamsız. Çevrimdışı. Tamamen sana ait.</p>`,
    );
    $("start").onclick = () => {
      if (activeRun) {
        continueRun();
