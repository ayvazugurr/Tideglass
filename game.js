(() => {
  ...
  function burst(removed) {
    ...
    for (const g of removed) {
      ...
      particles.push({x: pos.x, y: pos.y, vx: 0, vy: 0, life: 0.32, maxLife: 0.32, color: "#d0fff5", r: cell * 0.12, ring: true});
      for (let j = 0; j < 12; j++)
        particles.push({
          x: pos.x,
          y: pos.y,
          vx: Math.cos(j * Math.PI / 6) * (60 + Math.random() * 90),
          vy: Math.sin(j * Math.PI / 6) * (60 + Math.random() * 90) - 35,
